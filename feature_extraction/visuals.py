...
def save_waveform(y: np.ndarray, sr: int, path: str | Path, segments: list | None = None,
    ...
    ax.plot(t[::step], y[::step], color="#38bdf8", linewidth=0.6)
    if segments:
        for (s, e) in segments:
            ax.axvline(s, color="#475569", linewidth=0.5, linestyle=":")
    if highlight:
        ax.axvspan(highlight[0], highlight[1], color="#f97316", alpha=0.25)
