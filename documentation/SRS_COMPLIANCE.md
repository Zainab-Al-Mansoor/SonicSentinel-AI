...
| 14 | Critical-event recognition + distinguish similar pairs (gunshot vs fireworks/backfire, scream vs shouting, aggression vs conversation, glass vs metal, alarm vs horn, fault vs normal machinery, help vs speech) | ✅ | (1) look-alike recordings trained as Background Noise (fireworks, door knock, can opening, ordinary voices, calm film audio, MIMII normal machinery); (2) **look-alike check in every decision** (`LOOKALIKE_PAIRS`, Admin → `lookalike_margin`): a close call goes to manual review “Possible look-alike: could be …”, a Background decision with a strong critical score is flagged as a possible hidden event; (3) per-pair evaluation `scripts/lookalike_report.py` → `reports/lookalike_pairs.md`. Weak pairs: scream vs voices (33 % of voices mistaken), glass vs metal (only 2 metal clips); vehicle backfire has no licensed data yet – `import_local_downloads.py --sources lookalikes` adds all ESC-50 look-alikes |
...
| 4 | Accuracy for **both** models: ≥ 85 % accuracy, macro-F1 ≥ 0.80, critical recall ≥ 85 % | ⚠️ | Python: accuracy **0.865 ✅**, macro-F1 **0.863 ✅**, recall Gunshot 0.93 ✅, Panic Scream 0.93 ✅, Aggression 0.89 ✅, Help 1.00 ✅, Glass 0.83 (5 of 6 clips). **GTM run 3: accuracy 0.606, macro-F1 0.607** (below 85 % – the 1-s speech-commands transfer model tops out around 0.6–0.65 on these sounds, documented as a limitation). **Combined system (Python 0.7 + GTM 0.3): accuracy 0.868 ✅, macro-F1 0.866 ✅** |
...
| 6 | Model prediction & confidence comparison report (≥ 100 unseen clips, ≥ 10 per class) | ⚠️ | generated on all **695 unseen test clips** with every SRS column + summary + per-class + both confusion matrices (`reports/model_comparison_report.xlsx`, GTM run 3, python_weight 0.7); Glass Breaking has only 6 test clips (SRS asks ≥ 10) |
